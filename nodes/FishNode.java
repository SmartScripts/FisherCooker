...
public class FishNode extends Node{
	...
	private  final int FISHING_SPOT_ID = 328;
	...
	public void execute() {		
		...
		if(fishingSpot != null){
			Methods.getOnScreen(fishingSpot);
	...
	private boolean dropFish(){
		while(Inventory.contains(VARS.COOKED_FISH_IDS[0])){
			if(EAT_TROUT_ACTIONBAR != null && EAT_TROUT_ACTIONBAR.click(false)){
		...
		while(Inventory.contains(VARS.COOKED_FISH_IDS[1])){
			if(EAT_SALMON_ACTIONBAR != null && EAT_SALMON_ACTIONBAR.click(false)){
				Timer t = new Timer(2000);
				while(!Menu.isOpen() && t.isRunning()){
					Task.sleep(50,75);
				}
